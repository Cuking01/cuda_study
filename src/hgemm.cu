...
namespace hgemm_v6_impl
{

__global__ __launch_bounds__(384,1) static void v6_impl(const __grid_constant__ CUtensorMap tensor_map_A,const __grid_constant__ CUtensorMap tensor_map_B,half*c,u2 M,u2 K)
{
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1200
    const u2 tid=threadIdx.x;
    const u2 bx=blockIdx.x;
...
void hgemm_v6(cudaStream_t stream,const half* a, const half* b, half* c, u2 n, u2 m, u2 k)
{
    assert_throw(m%128==0&&n%128==0&&k%128==0,"m,n,k must be divisible by 128");
	...
	dim3 grid(k/128,n/128);
	dim3 block(256+128);
	hgemm_v6_impl::v6_impl<<<grid,block,0,stream>>>(tma_desc_A,tma_desc_B,c,m,k);
}
